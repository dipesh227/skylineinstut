...
export default function VerifyPage() {
  ...
  const handleVerify = async (roll: string = rollNumber) => {
    ...
    setLoading(true);
    setSearched(true);
    const { data, error } = await supabase.from("students").select("*").ilike("roll_number", roll.trim()).single();
    if (data) {
      const full: Student = { ...data, attendance_records: [], results_records: [], fee_ledgers: [] };
      setStudent(full);
        ...
        <div className="bg-white rounded-3xl border p-6 md:p-8 shadow-xl max-w-2xl mx-auto space-y-5">
          <div className="space-y-2 text-left">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="e.g. STL-2026-001" value={rollNumber} onChange={e => setRollNumber(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" /></div>
            ...
            {student ? (
              <>
                <div className="bg-emerald-50 border border-emerald-150 rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-4 max-w-2xl mx-auto">
                  ...
                  <div className="text-left text-xs text-emerald-800">We confirm that <strong>{student.name}</strong> is registered in our academic registry.</div>
                </div>
                <DegreeCertificate student={student} settings={settings} qrCodeBase64={qrCodeBase64} />
                <button onClick={() => downloadCertificatePdf(student, settings, qrCodeBase64)} className="mx-auto flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white text-xs font-bold rounded-xl shadow-md"><Download className="w-4 h-4" /> Download Verified PDF</button>
              </>
            ) : (
