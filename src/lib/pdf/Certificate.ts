...
export function downloadCertificatePdf(student: Student, settings?: SiteSettings | null, qrCodeBase64?: string): void {
  ...
  if (logoBase64) { addImageSafe(doc, logoBase64, 20, 20, 42, 21); }
  else { doc.setDrawColor('#D97706'); doc.circle(28, 28, 6, 'S'); doc.setFillColor('#D97706'); doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.text('S', 28, 30.5, { align: 'center' }); }
  ...
  doc.text(institutionName.toUpperCase(), 148.5, 46, { align: 'center' });
  ...
  doc.setFillColor('#FFFFFF'); doc.rect(136.5, sealY-2, 24, 24, 'F');
  doc.setDrawColor('#F59E0B'); doc.rect(136.5, sealY-2, 24, 24, 'S');
  if (qrCodeBase64) { addImageSafe(doc, qrCodeBase64, 138, sealY-0.5, 21, 21, 'PNG'); }
