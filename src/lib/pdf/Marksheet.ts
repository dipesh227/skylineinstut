...
export function downloadResultsPdf(student: Student, settings?: SiteSettings | null): void {
  ...
  const leftX = 15, rightX = 195, contentW = 180; let currentY = 15;
  const institutionName = settings?.institute_name || 'SKYLINE INSTITUTE';
  ...
  const photoW = 24, photoH = 30, photoX = 166, photoY = 15;
  ...
  if (logoBase64) { addImageSafe(doc, logoBase64, leftX, currentY, 24, 12); }
  doc.setTextColor('#1E3A8A'); doc.setFont('helvetica', 'bold'); doc.setFontSize(16);
  doc.text(institutionName.toUpperCase(), leftX, currentY+16);
  ...
  if (results.length > 0) {
    ...
    doc.text('No modular exam records registered yet.', leftX+10, currentY+12);
    currentY += 20;
  }
  ...
  // Stamp
  const stampX2 = leftX+6, stampY2 = currentY;
  doc.saveGraphicsState();
  doc.setDrawColor(isPass ? '#16A34A' : '#DC2626');
  doc.rect(stampX2, stampY2, 56, 15, 'S'); doc.rect(stampX2+0.6, stampY2+0.6, 54.8, 13.8, 'S');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(isPass ? 9.5 : 8);
  doc.text(isPass ? 'RESULT: PASSED' : 'RESULT: FAIL', stampX2+28, stampY2+7.5, { align: 'center' });
  doc.setFontSize(5); doc.text(`MINIMUM PASS CRITERIA: 33%  |  SCORE: ${percentage.toFixed(1)}%`, stampX2+28, stampY2+11.5, { align: 'center' });
